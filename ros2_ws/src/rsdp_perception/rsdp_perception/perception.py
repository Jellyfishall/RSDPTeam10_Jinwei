...
class AttrVote:
    ...
    probs: deque = field(default_factory=lambda: deque(maxlen=10))
...
class PerceptionStableAttrsNode(Node):
    ...
    def _publish(self, frame_id: str, color_msg: Image, fps: float):
        block_msg = BlockPoseObservation()
        block_msg.header = color_msg.header
        block_msg.header.frame_id = frame_id
        block_msg.yolo_fps = float(fps)

        bin_msg = BinPoseObservation()
        bin_msg.header = color_msg.header
        bin_msg.header.frame_id = frame_id
        bin_msg.yolo_fps = float(fps)

        platform_msg = PlatformPoseObservation()
        platform_msg.header = color_msg.header
        platform_msg.header.frame_id = frame_id
        platform_msg.yolo_fps = float(fps)

        opening_msg = BinOpeningPoseObservation()
        opening_msg.header = color_msg.header
        opening_msg.header.frame_id = frame_id
        opening_msg.yolo_fps = float(fps)
        ...
        for tid, tr in sorted(self.tracks.items(), key=lambda kv: kv[0]):
            ...
            if tr.cls == "block":
                ...
                obj.id = tr.track_id
                obj.pose = pose
                obj.confidence = float(tr.conf_ema)
                obj.age = int(tr.age)
                ...
                obj.shape_confidence = 0.0 if blk_sconf is None else float(blk_sconf)
                obj.shape_votes = int(blk_sn)
                block_msg.observations.append(obj)
                ...
                obj.center_u = int(tr.last_center_px[0])
                obj.center_v = int(tr.last_center_px[1])
                platform_msg.observations.append(obj)
        ...
        self.pub_platforms.publish(platform_msg)
